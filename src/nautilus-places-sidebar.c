...
static GtkTreeIter
add_separator (NautilusPlacesSidebar *sidebar,
...
static GtkTreeIter
add_expander (NautilusPlacesSidebar *sidebar,
	      SectionType section_type)
{
	GtkTreeIter iter;
			   ...
			   SectionType section_type)
{
	...
	case SECTION_DEVICES:
		if (!sidebar->devices_header_added) {
			add_separator (sidebar, SECTION_DEVICES);
	...
	case SECTION_BOOKMARKS:
		if (!sidebar->bookmarks_header_added) {
			add_separator (sidebar, SECTION_BOOKMARKS);
	   ...
	   const char *tooltip)
{
	...
	gtk_tree_store_append (sidebar->store, &iter, parent_iter);
	gtk_tree_store_set (sidebar->store, &iter,
...
update_places (NautilusPlacesSidebar *sidebar)
{
	NautilusBookmark *bookmark;
	GtkTreeSelection *selection;
	GtkTreeIter last_iter, network_iter;
	GtkTreeModel *model;
	GVolumeMonitor *volume_monitor;
	GList *mounts, *l, *ll;
	GMount *mount;
	...
	/* network */
	network_iter = add_expander (sidebar, SECTION_NETWORK);

 	mount_uri = "network:///"; /* No need to strdup */
	icon = g_themed_icon_new (NAUTILUS_ICON_NETWORK);
	add_place (sidebar, &network_iter,
		   PLACES_BUILT_IN,
		   SECTION_NETWORK,
			 ...
			 gpointer user_data)
{
	...
	is_expanded = gtk_tree_view_row_expanded (sidebar->tree_view,
						  path);
